...
class Compound extends Updater
{
    ...
    private static void loadBases() throws IOException, SQLException
    {
        ...
        oldCompounds = getIntSet("select id from compound_bases where keep", 200000000);
    }
    ...
    private static void loadBiosystems() throws IOException, SQLException
    {
        ...
        IntPairSet oldBiosystems = getIntPairSet("select compound, biosystem from compound_biosystems", 50000000);

        processFiles("RDF/compound/general", "pc_compound2biosystem_[0-9]+\\.ttl\\.gz", file -> {
        ...
        batch("delete from compound_biosystems where compound = ? and biosystem = ?", oldBiosystems);
        batch("insert into compound_biosystems(compound, biosystem) values (?,?)", newBiosystems);
    }
    ...
    private static void loadComponents() throws IOException, SQLException
    {
        ...
        IntPairSet oldComponents = getIntPairSet("select compound, component from compound_components", 10000000);

        try(InputStream stream = getStream("RDF/compound/general/pc_compound2component.ttl.gz"))
        {
            new TripleStreamProcessor()
            {
                @Override
        ...
        batch("delete from compound_components where compound = ? and component = ?", oldComponents);
        batch("insert into compound_components(compound, component) values (?,?)", newComponents);
    }
    ...
    private static void loadDrugproducts() throws IOException, SQLException
    {
        ...
        IntTripletSet oldIngredients = getIntTripletSet(
                "select compound, ingredient_unit, ingredient_id from compound_active_ingredients", 20000);

        try(InputStream stream = getStream("RDF/compound/general/pc_compound2drugproduct.ttl.gz"))
        {
            new TripleStreamProcessor()
            {
                @Override
        ...
        batch("delete from compound_active_ingredients where compound = ? and ingredient_unit = ? and ingredient_id = ?",
                oldIngredients);
        batch("insert into compound_active_ingredients(compound, ingredient_unit, ingredient_id) values (?,?,?)",
                newIngredients);
    }
    ...
    private static void loadIsotopologues() throws IOException, SQLException
    {
        ...
        IntPairSet oldIsotopologues = getIntPairSet("select compound, isotopologue from compound_isotopologues",
                5000000);

        try(InputStream stream = getStream("RDF/compound/general/pc_compound2isotopologue.ttl.gz"))
        {
            new TripleStreamProcessor()
            {
                @Override
        ...
        batch("delete from compound_isotopologues where compound = ? and isotopologue = ?", oldIsotopologues);
        batch("insert into compound_isotopologues(compound, isotopologue) values (?,?)", newIsotopologues);
    }
    ...
    private static void loadParents() throws IOException, SQLException
    {
        ...
        IntPairSet oldParents = getIntPairSet("select compound, parent from compound_parents", 10000000);

        try(InputStream stream = getStream("RDF/compound/general/pc_compound2parent.ttl.gz"))
        {
            new TripleStreamProcessor()
            {
                @Override
        ...
        batch("delete from compound_parents where compound = ? and parent = ?", oldParents);
        batch("insert into compound_parents(compound, parent) values (?,?)", newParents);
    }
    ...
    private static void loadSameConnectivities() throws IOException, SQLException
    {
        ...
        IntPairSet oldIsomers = getIntPairSet("select compound, isomer from compound_same_connectivities", 5000000);

        try(InputStream stream = getStream("RDF/compound/general/pc_compound2sameconnectivity.ttl.gz"))
        {
            new TripleStreamProcessor()
            {
                @Override
        ...
        batch("delete from compound_same_connectivities where compound = ? and isomer = ?", oldIsomers);
        batch("insert into compound_same_connectivities(compound, isomer) values (?,?)", newIsomers);
    }
    ...
    private static void loadStereoisomers() throws IOException, SQLException
    {
        ...
        IntPairSet oldIsomers = getIntPairSet("select compound, isomer from compound_stereoisomers", 100000000);

        processFiles("RDF/compound/general", "pc_compound2stereoisomer_[0-9]+\\.ttl\\.gz", file -> {
            try(InputStream stream = getStream(file))
            {
                new TripleStreamProcessor()
                {
        ...
        batch("delete from compound_stereoisomers where compound = ? and isomer = ?", oldIsomers);
        batch("insert into compound_stereoisomers(compound, isomer) values (?,?)", newIsomers);
    }
    ...
    private static void loadRoles() throws IOException, SQLException
    {
        ...
        IntPairSet oldRoles = getIntPairSet("select compound, role_id from compound_roles", 20000);

        try(InputStream stream = getStream("RDF/compound/general/pc_compound_role.ttl.gz"))
        {
            new TripleStreamProcessor()
            {
                @Override
        ...
        batch("delete from compound_roles where compound = ? and role_id = ?", oldRoles);
        batch("insert into compound_roles(compound, role_id) values (?,?)", newRoles);
    }
    ...
    private static void loadTypes() throws IOException, SQLException
    {
        ...
        IntTripletSet oldTypes = getIntTripletSet("select compound, type_unit, type_id from compound_types", 200000);

        try(InputStream stream = getStream("RDF/compound/general/pc_compound_type.ttl.gz"))
        {
            new TripleStreamProcessor()
            {
                @Override
        ...
        batch("delete from compound_types where compound = ? and type_unit = ? and type_id = ?", oldTypes);
        batch("insert into compound_types(compound, type_unit, type_id) values (?,?,?)", newTypes);
    }
    ...
    {
        batch("delete from compound_bases where id = ? and "
                + "not exists (select id from molecules.pubchem where compound_bases.id = molecules.pubchem.id)",
                oldCompounds);

        batch("update compound_bases set keep = false where id = ? and "
                + "exists (select id from molecules.pubchem where compound_bases.id = molecules.pubchem.id)",
                oldCompounds);

        batch("insert into compound_bases(id,keep) values(?,true) on conflict (id) do update set keep=EXCLUDED.keep",
                newCompounds);
