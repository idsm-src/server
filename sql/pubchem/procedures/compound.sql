...
$$
  select distinct on (compound) compound, score, name from (
    ...
    union all
    select compound, similarity(preferred_iupac_name, query) as score, preferred_iupac_name as name
    ...
    union all
    select cmp.compound as compound, similarity(val.value, query) as score, val.value as name
